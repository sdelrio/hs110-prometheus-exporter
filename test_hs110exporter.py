...
class TestValidIP(unittest.TestCase):
    ...
    def test_ipvalues_hypo(self, ip_sub1, ip_sub2, ip_sub3, ip_sub4):
        ip_string = "%s.%s.%s.%s" % (str(ip_sub1), str(ip_sub2), str(ip_sub3), str(ip_sub4))
        self.assertEqual(ip_string, valid_ip(ip_string))
...
class TestHS110data(unittest.TestCase):
    def test_encryptstring(self):
        hs110 = HS110data()
        text_encrypted = (
            b'\x00\x00\x00\x10\xd0\xf0\x98\xfd\x91\xfd'
            b'\x92\xa8\x88\xff\x90\xe2\x8e\xea\xca\xb7'
        )
    ...
    def test_constructor(self, hardware):
        if (hardware in ['h1', 'h2']):
            ...
            hs110._HS110data__received_data['emeter']['get_realtime'][
                hs110._HS110data__keyname[hardware]['current']] = 1
            hs110._HS110data__received_data['emeter']['get_realtime'][
                hs110._HS110data__keyname[hardware]['voltage']] = 220
            hs110._HS110data__received_data['emeter']['get_realtime'][
                hs110._HS110data__keyname[hardware]['power']] = 220
            hs110._HS110data__received_data['emeter']['get_realtime'][
                hs110._HS110data__keyname[hardware]['total']] = 1.3
    ...
    def test_get_cmd(self):
        cmd_encrypted = (
            b'\x00\x00\x00\x1e\xd0\xf2\x97\xfa\x9f\xeb\x8e\xfc\xde\xe4\x9f\xbd\xda'
            b'\xbf\xcb\x94\xe6\x83\xe2\x8e\xfa\x93\xfe\x9b\xb9\x83\xf8\x85\xf8\x85'
        )
        hs110 = HS110data()
    ...
    def test_socket(
        ...
        hs110 = HS110data(ip=test_ip, port=test_port)
        sample_data = hs110._HS110data__encrypt(
            '{"emeter":{"get_realtime":{"voltage_mv":229865,"current_ma":'
            '1110,"power_mw":231866,"total_wh":228,"err_code":0}}}'
        )
        sample_data_dict = {
            'emeter': {'get_realtime': {
                'voltage_mv': 229865,
        ...
        with patch('builtins.print') as mock_print:
            ...
            self.assertEqual(
                hs110._HS110data__received_data, hs110._HS110data__empty_data())
    ...
    def test_main(self, mock_http_server, mock_connect, mock_sleep):
        ...
        parser.add_argument(
            "-t", "--target", metavar="<ip>", required=False,
            help="Target IP Address", default="192.168.1.1", type=str)
        parser.add_argument(
    ...
    def test_command_line(self):
        ...
        the_exception = command.exception
